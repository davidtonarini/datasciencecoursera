...
The effect of transmission on consumption, in Miles(US) / gallon, is analyzed. Data analysis indicates that cars with automatic transmission tend to have lower consumptions, reaching more Miles per gallon compared to cars with manual transmission.


## Exploratory analysis
```{r cars, echo=FALSE}
auto_cars <- mtcars[mtcars$am == 1, ]
man_cars <- mtcars[mtcars$am == 0, ]
...
For cars with automatic transmission, the mean consumption is `r round(mean(man_cars[ , 1]),2)` mpg ± `r round(sd(man_cars[ , 1]),2)`
...
ggplot(mtcars, mapping=m) + geom_point() + geom_smooth(mapping=m, method= "lm")
```
